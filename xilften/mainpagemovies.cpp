...
void mainpagemovies::on_seeAllC_clicked()
{
    ...
    if(query.first()){
        ...
        ui->tableView_6->setColumnWidth(1,347);
...
void mainpagemovies::on_delete_4_clicked()
{
    ...
    QString m = MainWindow::name;
    QString movieName = ui->Title_3->text();
    query.exec("SELECT id as ID, name AS Name, comment AS Comment from comment where movieName = '"+movieName+"' and name = '"+m+"' ");
    model->setQuery(query);
    if(query.first()){
        ui->tableView_7->setModel(model);
        ui->tableView_7->setColumnWidth(0,30);
        ui->tableView_7->setColumnWidth(1,150);
        ui->tableView_7->setColumnWidth(2,280);
...
void mainpagemovies::on_refresh_clicked()
{
    this->close();
    mainpagemovies m;
    m.setModal(true);
